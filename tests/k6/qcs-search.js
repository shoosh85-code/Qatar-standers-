...
export default function () {
  const res = http.post(
    ...
    JSON.stringify({ q: 'concrete compressive strength', section: 'Part 5' }),
    {
      headers: {
...
export function handleSummary(data) {
  ...
  console.log('=== qcs-search Rate Limit Test ===');
  ...
  console.log(`Free Limit     : 10 req/min — ${rl > 0 ? '✅ يعمل' : '❌ لا يعمل'}`);

  return { 'results/qcs-search.json': JSON.stringify(data) };
}
