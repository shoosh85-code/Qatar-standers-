...
export async function retryGemini(primaryFn, fallbackFn = null, opts = {}) {
  const geminiOpts = {
    retries:   2,
    baseDelay: 2000,  // ابدأ بـ 2 ثانية لـ Gemini
    maxDelay:  20000,
    factor:    2.5,
    jitter:    true,
    timeout:   28000,
  ...
  try {
    return await withRetry(primaryFn, geminiOpts, ({ attempt, error }) => {
      // تسجيل مفصّل لمشاكل Gemini
      if (error.message?.includes('quota') || error.message?.includes('429')) {
        console.warn(`[retryGemini] Quota hit (attempt ${attempt}) — waiting with backoff`);
      }
    });
  } catch (primaryError) {
    // إذا فشل الـ primary بعد كل المحاولات → جرّب الـ fallback
    if (fallbackFn) {
      console.warn('[retryGemini] Primary فشل نهائياً — جاري تجربة fallback');
      try {
        ...
        console.error('[retryGemini] Fallback أيضاً فشل:', fallbackError.message);
        // ارمِ خطأ الـ primary (الأكثر وصفاً)
        throw primaryError;
      }
    }
    throw primaryError;
